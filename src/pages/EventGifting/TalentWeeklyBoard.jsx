...
// import { talentWeeklyNow, talentWeeklyPrev } from "../../utils/testData";
import Topper from "../../components/Topper";
import GiftingLeaderboardItem from "../../components/GiftingLeaderboardItem";
import { AppContext } from "../../AppContext";

const TalentWeeklyBoard = () => {
  const { giftingLeaderboard } = useContext(AppContext);
  const { weeklyTalentNow, weeklyTalentPrev } = giftingLeaderboard;

  // console.log("weekly talent now:", weeklyTalentNow);
  ...
  useEffect(() => {
    if (switchTabs.current) {
      setSelectedData(weeklyTalentNow);
    } else {
      setSelectedData(weeklyTalentPrev);
    }
  ...
  return (
    <div className="board">
      ...
      {switchTabs.current && weeklyTalentNow.length > 10 ? (
        <button
      ...
      {switchTabs.prev && weeklyTalentNow.length > 10 ? (
        <button
          className={seeMore ? "seeMore" : "seeLess"}
          onClick={toggleSeeMore}
        />
      ...
      {switchTabs.current && weeklyTalentNow?.length === 0 && (
        <div className="noDataFound">No Data Found</div>
      )}
      {switchTabs.prev && weeklyTalentPrev?.length === 0 && (
        <div className="noDataFound">No Data Found</div>
      )}
    </div>
  );
...
export default TalentWeeklyBoard;
