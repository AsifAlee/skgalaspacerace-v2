...
const UserRoundLeaderBoard = ({ round }) => {
  const { roundsLbData, info } = useContext(AppContext);
  const { myRound1, myRound2, myRound3 } = roundsLbData;

  const [seeMore, setSeeMore] = useState(true);
  const [selectedData, setSelectedData] = useState(myRound1);

  useEffect(() => {
    if (round === 1) {
      setSelectedData(myRound1);
    } else if (round === 2) {
      setSelectedData(myRound2);
    } else if (round === 3) {
      setSelectedData(myRound3);
    }
  }, [roundsLbData, round]);
...
export default UserRoundLeaderBoard;
